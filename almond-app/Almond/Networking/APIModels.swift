...
/// POST /healthkit response — status: "pending" signals non-blocking processing.
/// Poll GET /risk?upload_id=<id> until status == "done".
struct HealthKitUploadResponse: Decodable {
    let uploadId: String
    let receivedAt: String
    let status: String   // "pending" | "scoring" | "recommending" | "done" | "failed"
    enum CodingKeys: String, CodingKey {
...
/// Discriminated union for GET /risk: either a full result or a not-yet-done stub.
enum RiskPollResponse: Decodable {
    case pending(uploadId: String, status: String)
    case failed(uploadId: String)
    case done(RiskResponseFull)

    private enum CodingKeys: String, CodingKey {
        case status, uploadId = "upload_id"
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        switch status {
        case "done":
            self = .done(try RiskResponseFull(from: decoder))
        case "failed":
            let id = try c.decode(String.self, forKey: .uploadId)
            self = .failed(uploadId: id)
        default:
            let id = try c.decode(String.self, forKey: .uploadId)
            self = .pending(uploadId: id, status: status)
        }
    }
}

struct RiskResponseFull: Decodable {
    let uploadId: String
    let status: String
...
struct RiskScores: Decodable {
    ...
    let nhanesMortality2yr: NHANESScore?   // nhanes_mortality_2yr per Phase-1 lock
    enum CodingKeys: String, CodingKey {
...
struct RiskDriver: Decodable, Identifiable {
    ...
    let direction: String       // "worse" | "better"
    let weight: Double
    let humanLabel: String
    let source: String          // "cox" | "augmentation"
    enum CodingKeys: String, CodingKey {
        case feature, value, direction, weight, source
...
enum AlmondError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .pollTimeout: return "Risk computation is taking longer than expected. Try again later."
        }
    }
}
