...
final class HealthKitManager {
    ...
    private static let readTypes: Set<HKObjectType> = {
        var types: Set<HKObjectType> = [
            ...
            // Characteristics (read via HKHealthStore convenience methods)
            HKQuantityType(.height),
            HKQuantityType(.bodyMass),
            // Clinical enrichment (optional — include if available)
    ...
    func buildUploadPayload() async throws -> HealthKitUploadRequest {
        ...
        async let exercise = queryDailyQuantity(type: .appleExerciseTime,        unit: .minute(),                            options: .cumulativeSum,   start: windowStart, end: now)
        async let sleep    = querySleepSessions(start: windowStart, end: now)
        async let rhr      = queryDailyQuantity(type: .restingHeartRate,         unit: .count().unitDivided(by: .minute()),  options: .discreteAverage, start: windowStart, end: now)
        async let hrv      = queryTimestampedQuantity(type: .heartRateVariabilitySDNN, unit: .secondUnit(with: .milli),                                start: windowStart, end: now)
        async let vo2      = queryMostRecentQuantity(type: .vo2Max,              unit: HKUnit(from: "ml/kg·min"),                                      start: windowStart, end: now)
        async let whr      = queryDailyQuantity(type: .walkingHeartRateAverage,  unit: .count().unitDivided(by: .minute()),  options: .discreteAverage, start: windowStart, end: now)

        let (stepSamples, energySamples, exerciseSamples, sleepResult,
             rhrSamples, hrvSamples, vo2Result, whrSamples)
            = await (steps, energy, exercise, sleep, rhr, hrv, vo2, whr)

        return HealthKitSamples(
            ...
            restingHrDaily:       rhrSamples.map     { RestingHRSample(date: dayFmt.string(from: $0.date), bpm: Int($0.value.rounded())) },
