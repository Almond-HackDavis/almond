...
6. **If your PR has merge conflicts**: rebase your branch on the latest `main` _locally_, push the rebased branch with `--force-with-lease` (never plain `--force`), and re-tag `@3arii`. Do not resolve conflicts in the GitHub web UI.
...
A passive long-term health-risk app. The user wears their Apple Watch normally, fills out a 30-second onboarding form once, and the app continuously computes risk scores (10-year cardiovascular event risk, 10-year diabetes risk, fitness age, AHA Life's Essential 8, **2-year NHANES-trained all-cause mortality risk**) using the Watch's HealthKit data. A trained ML model plus published clinical risk equations produce the scores; Gemini turns the score breakdown into specific, personalized lifestyle recommendations.

> **Why 2 years and not 10?** The publicly available NHANES Public-Use Linked Mortality File (2019 vintage) caps follow-up at ~7 years post-interview for cycle H (2013–2014) and ~9 years for cycle G (2011–2012). Calibrating a _10-year_ hazard from this data is dishonest; 2-year is the longest horizon that doesn't require Kaplan–Meier extrapolation past observed support. The published clinical equations (ASCVD, Framingham, FINDRISC) keep their original 10-year horizon because those windows are baked into the equations themselves — only the NHANES-trained model is 2-year.
...
| ML model          | scikit-survival `CoxPHSurvivalAnalysis` trained on NHANES wrist-actigraphy (cycles G+H, 2011–2014) + 2019 NDI linked mortality, **2-yr all-cause mortality endpoint**    | Saved as `almond-ml/models/cox_model.pkl` + `almond-ml/models/feature_means.json` for imputation defaults |
...
## Locked ML feature vector — do not change without a `feat/` PR

The trained Cox model accepts **exactly 8 features** in this order. iOS's `POST /healthkit` payload + the onboarding row must produce all 8 at inference time. The same vector is what `02_features.ipynb` engineers from NHANES.

| #   | Feature              | Source at training (NHANES)                       | Source at inference (HealthKit + onboarding)                               | Units    |
| --- | -------------------- | ------------------------------------------------- | -------------------------------------------------------------------------- | -------- |
| 1   | `age`                | `DEMO.RIDAGEYR`                                   | onboarding `age` (or `HKCharacteristicTypeIdentifierDateOfBirth`)          | years    |
| 2   | `sex_male`           | `DEMO.RIAGENDR == 1`                              | onboarding `sex == "M"`                                                    | bool     |
| 3   | `bmi`                | `BMX.BMXBMI`                                      | derived from onboarding `height_cm`, `weight_kg`                           | kg/m²    |
| 4   | `mean_daily_mims`    | mean of `PAXDAY.PAXAISMD` across valid wear-days  | proxy from `steps_daily` + `active_energy_daily_kcal` (mapping in `ml.py`) | MIMS/day |
| 5   | `sd_daily_mims`      | stdev of `PAXDAY.PAXAISMD` across valid wear-days | stdev of the same daily proxy                                              | MIMS/day |
| 6   | `mean_sleep_min`     | mean of `PAXDAY.PAXSWMD`                          | mean of `sleep_sessions.duration_min` over the upload window               | minutes  |
| 7   | `sd_sleep_min`       | stdev of `PAXDAY.PAXSWMD`                         | stdev of the same                                                          | minutes  |
| 8   | `mean_wake_wear_min` | mean of `PAXDAY.PAXWWMD`                          | total daily on-wrist minutes implied by HK sample density                  | minutes  |

**Valid-day filter** (training and any inference re-aggregation): `PAXSTS == 1` (reliable monitor) ∧ `PAXWWMD ≥ 600` (≥10 h wake-wear) ∧ `PAXQFD == 0` (no quality flag) ∧ ≥4 valid days per subject. SAS sentinel `5.397605e-79` must be coerced to `0.0` before any arithmetic.

## Augmentation-only signals (NOT learned, multipliers from published literature)

These are critical to the user-facing scores but **cannot** be trained because the public NHANES wearable release contains no heart-rate, HRV, VO2-max, or sleep-stage data. They enter the pipeline via `ml.apply_augmentation()` after the Cox prediction.

| Signal               | Multiplier source                                               | Applied to       |
| -------------------- | --------------------------------------------------------------- | ---------------- |
| Resting HR           | Jensen 2013 (HR 1.16 per +10 bpm, all-cause mortality)          | Cox 2-yr hazard  |
| HRV (SDNN)           | Hillebrand 2013 meta-analysis (HR 1.10 per −10 ms)              | Cox 2-yr hazard  |
| VO2 max              | FRIEND registry percentiles by age × sex; mapped to fitness-age | `fitness_age`    |
| Sleep Regularity Idx | Phillips 2017 SRI computed from HK minute-level sleep           | LE8 sleep-health |
| Sleep stages         | not used as training feature; surfaced for display + LE8 only   | LE8 sleep-health |

If a signal is missing in the upload, its multiplier is `1.0` and the augmented hazard equals the raw Cox hazard — this is the documented degradation mode.

## Training data manifest

All files land under `almond-ml/training/data/` (gitignored). `01_download_nhanes.ipynb` pulls them idempotently.

| File                                    | URL pattern                                                                          |
| --------------------------------------- | ------------------------------------------------------------------------------------ |
| `DEMO_G.xpt`, `BMX_G.xpt`               | `https://wwwn.cdc.gov/Nchs/Data/Nhanes/Public/2011/DataFiles/{name}.xpt`             |
| `DEMO_H.xpt`, `BMX_H.xpt`               | `https://wwwn.cdc.gov/Nchs/Data/Nhanes/Public/2013/DataFiles/{name}.xpt`             |
| `PAXHD_G.xpt`, `PAXDAY_G.xpt`           | same 2011 path                                                                       |
| `PAXHD_H.xpt`, `PAXDAY_H.xpt`           | same 2013 path                                                                       |
| `NHANES_2011_2012_MORT_2019_PUBLIC.dat` | `https://ftp.cdc.gov/pub/Health_Statistics/NCHS/datalinkage/linked_mortality/{name}` |
| `NHANES_2013_2014_MORT_2019_PUBLIC.dat` | same                                                                                 |

**Do not** attempt to download `PAXMIN_*` (minute-level) — CDC does not publicly distribute it as XPT, the URL returns 404. The minute-level signals AGENTS used to imply (sleep stages, MVPA cutpoints, true SRI) are not learnable from this dataset; they are augmentation-only per the table above.
...
4. **Backend processing** (synchronous within the request, ~2–4 seconds expected):
   ...
   4. Run NHANES Cox model → **2-year all-cause mortality hazard** — `ml.predict_cox()`
...
```json
{
  ...
  "top_drivers": [
    ...
    {
      "feature": "sleep_regularity_index",
      "value": 58,
      "population_norm": 75,
      "direction": "worse",
      "weight": 0.27,
      "human_label": "Sleep regularity",
      "source": "augmentation"
    },
    {
      "feature": "vo2_max",
      ...
      "direction": "worse",
      "weight": 0.22,
      "human_label": "Cardiorespiratory fitness",
      "source": "augmentation"
    }
  ],
...
```json
{
  ...
  "series": {
    ...
    "nhanes_mortality_2yr": [{ "date": "2026-04-18", "value": 0.6 }],
...
- **ML model**: `cox_model.pkl` loads in <1 s, predictions deterministic for fixed inputs, validation concordance index ≥ 0.70 for the **24-month all-cause mortality endpoint** on a held-out 25% split (stratified by event), pooled cohort = NHANES cycles G + H adults with ≥4 valid wear-days. Training notebook reproducible (fixed seed + pinned package versions). Cohort-construction sanity numbers reproduced by `inspect/inspect_nhanes.ipynb` for cycle H alone: 4,350 subjects, 77 deaths within 24 months (~9.6 events/variable on the 8-feature vector); pooling cycle G is expected to roughly double both.
